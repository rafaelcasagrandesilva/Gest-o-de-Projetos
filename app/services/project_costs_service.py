...
class ProjectCostsService:
    ...
    async def create(self, *, actor_user_id, data: dict) -> ProjectCost:
    ...
    async def update(self, *, actor_user_id, cost_id: UUID, data: dict) -> ProjectCost:
    ...
    async def delete(self, *, actor_user_id, cost_id: UUID) -> None:
        row = await self.repo.get(cost_id)
        if not row:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Custo não encontrado.")
        before = model_to_dict(row)
